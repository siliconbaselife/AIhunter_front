    ...
    closeAllMsgDivs = async() => {
        ...
        while(msgDivs.length > 0) {
            for (let msgDiv of msgDivs) {
                let [closeBtn] = await msgDiv.$x(`//button[contains(@class, "msg-overlay-bubble-header__control") and not(contains(@class, "msg-overlay-conversation-bubble__expand-btn"))]`);
